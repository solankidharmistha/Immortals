    ...
    print('dashboard call');
     _stream = FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if(_type == 'new_order' || _type == 'order_request') {
        ...
        Get.dialog(NewRequestDialog(isRequest: true, onTap: () => _navigateRequestPage(), orderId: int.parse(_orderID)));
      }else if(_type == 'assign' && _orderID != null && _orderID.isNotEmpty) {
        Get.find<OrderController>().getCurrentOrders();
        Get.find<OrderController>().getLatestOrders();
        Get.dialog(NewRequestDialog(isRequest: false, onTap: () => Get.toNamed(RouteHelper.getOrderDetailsRoute(int.parse(_orderID))), orderId: int.parse(_orderID)));
      }else if(_type == 'block') {
